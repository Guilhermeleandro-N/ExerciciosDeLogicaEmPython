...
for c in range(5):
    ...
    x=input("Nome: ")
    veiculos.append(x)
    x=float(input("Km po litro: "))
    km_litro.append(x)
    y=1000/x
    quantidade_litro.append(y)
    z=y*2.25
    valor.append(z)
    if maior<km_litro[c]:
        maior=km_litro[c]
...
for c in range(5):
    print(f"{c+1} - {veiculos[c]}           -    {round(km_litro[c],2)} -  {round(quantidade_litro[c],2)} litros - R$ {round(valor[c],2)}")
print(f"O menor consumo é de {veiculos[indice]}")
input("Pressione um tecla qualquer...")